...
int main()
{
    ...
    printf("ML-DSA-44 Keypair generation starting...\n");
    ...
    if (ret == 0)
    {
        printf("Keypair generation successful!\n");

        // 생성된 공개키의 첫 16 바이트 출력 (확인용)
    ...
    {
        printf("Keypair generation failed! Error code: %d\n", ret);
        return -1; // 키 생성 실패 시 후속 작업 중단
    }

    // 서명할 원본 메시지와 길이 설정
    const uint8_t m[] = "Hello, ML-DSA-44!";
    size_t mlen = strlen((const char *)m);

    // 도메인 분리(Domain Separation)를 위한 컨텍스트 설정
    const uint8_t ctx[] = "TestContext";
    size_t ctxlen = strlen((const char *)ctx);

    // 서명된 메시지를 저장할 버퍼와 결과 길이를 저장할 변수 할당
    // 버퍼의 크기는 (최대 서명 길이 + 원본 메시지 길이) 이상이어야 합니다.
    uint8_t sm[PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES + sizeof(m)];
    size_t smlen = 0;

    printf("Signature generation starting...\n");

    // 컨텍스트 기반 서명 생성 함수 호출
    ret = PQCLEAN_MLDSA44_CLEAN_crypto_sign_ctx(sm, &smlen, m, mlen, ctx, ctxlen, sk);
    
    if (ret == 0)
    {
        printf("Signature generation successful!\n");
        printf("Total signed message length: %zu bytes\n", smlen);
    }
    else
    {
        printf("Signature generation failed! Error code: %d\n", ret);
    }
